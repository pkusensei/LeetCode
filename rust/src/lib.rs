...
pub fn max_xor(nums: &[i32], k: i32) -> i32 {
    ...
    for (right, &num) in nums.iter().enumerate() {
        ...
        while let Some(&(left1, minv)) = minq.front()
            ...
            && maxv - minv > k
        {
            // min(left1, left2) must be dropped from window
            while left <= left1.min(left2) {
                trie.update(pref_xor[left], -1);
                left += 1;
            }
            // Now `left` points to start of valid window
            // pref_xor[left] should be considered when xoring curr_xor
            // curr_xor ^ pref_xor[left] removes nums[..left]
            // giving xor of nums[left..=right]
