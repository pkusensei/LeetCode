    ...
    for (right, num) in nums.iter().enumerate() {
        ...
        while curr >= target {
            if let Some(v) = res.as_mut() {
                *v = (*v).min(right - left + 1)
            } else {
                res = Some(right - left + 1)
            }
            curr -= nums[left];
            left += 1;
        }
    }
    res.unwrap_or(0) as _
}
...
mod tests {
    ...
    #[test]
    fn basics() {
        debug_assert_eq!(min_sub_array_len(7, &[2, 3, 1, 2, 4, 3]), 2);
        debug_assert_eq!(min_sub_array_len(4, &[1, 4, 4]), 1);
        debug_assert_eq!(min_sub_array_len(11, &[1, 1, 1, 1, 1, 1, 1, 1]), 0);
    }

    #[test]
    fn test() {
        debug_assert_eq!(min_sub_array_len(11, &[1, 2, 3, 4, 5]), 3);
        debug_assert_eq!(
            min_sub_array_len(
                697439,
                &[
